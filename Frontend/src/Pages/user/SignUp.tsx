...
const SignUp: FC = () => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    password: Yup.string().required('Password is required').matches(/^(?=.*[A-Z]).{8,}$/, 'Password must contain at least one capital letter and be at least 8 characters long')
  });

  const handleSubmit = () => {

    validationSchema.validate({ name, email, phoneNumber, password }, { abortEarly: false }).then(() => {
      axiosInstance.post('/signUp', { name: name, email: email, phoneNumber:phoneNumber, password: password }).then((response) => {
  ...
  return (
    <GoogleOAuthProvider clientId="684475057007-2abci7ht9ppi3313g5duloq7um38qlp3.apps.googleusercontent.com">
      <div>
        <h1 className="text-3xl lg:text-4xl font-bold text-center mt-5 dark:text-white">
          MATCH<span className="text-3xl lg:text-4xl font-bold text-center text-pink-700 mt-6 lg:mt-16">MAKER</span>
        </h1>
        ...
        <div className="container lg:w-8/12 mobile:w-10/12  xl:w-4/12 mx-auto px-4 mt-6 lg:mt-10 bg-pink-100 p-8 rounded-lg shadow-lg">
          ...
          <form className="flex flex-col items-center">
            ...
            <div className="mb-6  w-4/5">
              <label htmlFor='email' className="block text-base lg:text-lg font-medium text-gray-800 mb-2">Phone Number</label>
              <input
                type="tel"
                className="block w-full lg:px-4 lg:py-2  text-lg lg:text-xl bg-white border border-gray-300 rounded focus:outline-none focus:ring focus:ring-pink-500 focus:border-pink-500 dark:text-black"
              ...
              <div className='flex w-full justify-center items-center'>
                <GoogleLogin
                  onSuccess={credentialResponse => {
                    // console.log(credentialResponse, "cred response")
